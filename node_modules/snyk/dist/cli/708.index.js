...
exports.generateTags = exports.generateProjectAttributes = void 0;
...
function getProjectAttribute(attribute, permitted, options) {
    ...
    if (options[attribute] === true) {
        throw new Error(`--${attribute} must contain an '=' with a comma-separated list of values. To clear all existing values, pass no values i.e. --${attribute}=`);
    }
    const values = options[attribute].split(',');
    const extra = values.filter((value) => !permittedValues.includes(value));
    if (extra.length > 0) {
        throw new Error(`${extra.length} invalid ${attribute}: ${extra.join(', ')}. ` +
...
function generateProjectAttributes(options) {
    return {
        criticality: getProjectAttribute('business-criticality', types_1.PROJECT_CRITICALITY, options),
        environment: getProjectAttribute('environment', types_1.PROJECT_ENVIRONMENT, options),
        lifecycle: getProjectAttribute('lifecycle', types_1.PROJECT_LIFECYCLE, options),
    };
}
...
function generateTags(options) {
    ...
    if (options.tags === true) {
        throw new Error(`--tags must contain an '=' with a comma-separated list of pairs (also separated with an '='). To clear all existing values, pass no values i.e. --tags=`);
    }
    const tags = [];
    ...
    for (const keyEqualsValue of keyEqualsValuePairs) {
        ...
        if (parts.length !== 2) {
            throw new Error(`The tag "${keyEqualsValue}" does not have an "=" separating the key and value.`);
        }
        tags.push({
            key: parts[0],
            value: parts[1],
